"""Figure 6: real-time onset-anchored PMO over the 2020 Equateur EVD outbreak.
...
Loads pre-computed results from results/fig6_pmo_realtime_equateur2020.csv (run
...
def main() -> None:
    set_style()
    df = pd.read_csv(RESULTS_DIR / "fig6_pmo_realtime_equateur2020.csv")
    ...
    fig = realtime_two_panel(
        ...
        prior_sse=FIG6_PRIOR_SSE,
        highlight_week=FIG6_RESPONSE_WEEK,
        highlight_label="declaration (~1 Jun)",
        pmo_legend_loc="lower right",
    )
    save(fig, "fig6_pmo_realtime_equateur2020", OUT_DIR)
