"""Compute and save results for Figure 3: real-time PMO, simulated SSI outbreak.
...
from analysis._shared.realtime import realtime_ensemble
...
def main() -> None:
    ...
    counts = simulate_ssi_delay(
        ...
        t_max=SIM_T_MAX,
        rng=np.random.default_rng(FIG3_SIM_SEED),
    )
    print(
        f"simulated SSI onset counts (seed {FIG3_SIM_SEED}): "
    ...
    cols = realtime_ensemble(
        ...
        tost=tost,
        inc=inc,
        n_particles=FIG3_N_PARTICLES,
        threshold=SIM_THRESHOLD,
        t_max=SIM_T_MAX,
        prior_sse=FIG3_PRIOR_SSE,
        rng=np.random.default_rng(SIM_SEED),
    )
    df = pd.DataFrame(cols)

    RESULTS_DIR.mkdir(parents=True, exist_ok=True)
    out_path = RESULTS_DIR / "fig3_pmo_realtime_sim.csv"
