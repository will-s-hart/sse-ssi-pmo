"""Figure 12: onset-anchored PMO comparison across symptom-onset histories.
...
Loads pre-computed results from results/fig12_pmo_comparison_delay.csv (run
...
def main() -> None:
    ...
    df = pd.read_csv(RESULTS_DIR / "fig12_pmo_comparison_delay.csv")
    n = len(df)
    bar_width = 0.35
    x = np.arange(n)
    ...
    save(fig, "fig12_pmo_comparison_delay", OUT_DIR)
