...
Companion to Figure 14 (the real Likati outbreak) on a weekly onset history
...
Loads pre-computed results from results/fig13_pmo_realtime_sim.csv (run
...
def main() -> None:
    set_style()
    df = pd.read_csv(RESULTS_DIR / "fig13_pmo_realtime_sim.csv")
    ...
    fig = realtime_two_panel(df, title=title, xtick_labels=xtick_labels, prior_sse=FIG13_PRIOR_SSE)
    save(fig, "fig13_pmo_realtime_sim", OUT_DIR)
