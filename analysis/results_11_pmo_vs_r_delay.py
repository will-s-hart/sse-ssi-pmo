"""Compute and save results for Figure 11: onset-anchored PMO vs r.

Symptom-onset-anchored analogue of Figure 1. The observed history is symptom
onsets: an index onset followed by ``r`` weeks without onsets. PMO is estimated
by simulation only (rejection sampling), for the onset-anchored SSE and SSI
models. See ``notes/notes.tex`` §"Symptom-onset data and delayed transmission".
"""
...
def main() -> None:
    ...
    sse_sim = np.empty(r_vals.size, dtype=np.float64)
    ssi_sim = np.empty(r_vals.size, dtype=np.float64)
    rng = np.random.default_rng(SIM_SEED)
    for i, r in enumerate(tqdm(r_vals, desc="fig11 sims")):
    ...
    out_path = RESULTS_DIR / "fig11_pmo_vs_r_delay.csv"
    df = pd.DataFrame(
        {
            "r": r_vals,
