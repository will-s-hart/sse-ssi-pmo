...
def weekly_onsets(date_strings: list[str]) -> tuple[np.ndarray, list[str]]:
    ...
    Returns ``(counts, week_starts)`` where ``counts[w]`` is the number of onsets
    in week ``w`` (week 0 = the calendar week of the first onset) and
...
def main() -> None:
    ...
    counts, week_starts = weekly_onsets(FIG14_ONSET_DATES)
